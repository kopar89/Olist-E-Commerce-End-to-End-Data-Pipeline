...
│  9 пар тасков (upload → load  │
│  Пары работают параллельно    │
│  Внутри пары: последовательно │
└──────────┬────────────────────┘
           │
           ▼
    CSV → Parquet → MinIO (S3)    ← Data Lake (upload таск)
           │
           ▼
    MinIO (S3) → PostgreSQL       ← DWH (load таск)
           │
           ▼
┌─────────────────────┐
